...
def generate(model, input_ids, tokenizer):
    sampling_params = {"temperature": 0, "top_p": 1, "max_new_tokens": 256}
    input_ids = input_ids.clone()
    input_ids = input_ids[0].tolist()
    output = model.generate(input_ids = input_ids, sampling_params = sampling_params)
    output = tokenizer(output['text'], return_tensors="pt")['input_ids'] # compatible with TA's code
...
def main():
    ...
    model_name = "Jess437/Llama-3.2-1B-Instruct-distill-gptq-b8g128"
    model = sgl.Engine(model_path=model_name,
                       context_length=2048,
                    ...
                    #    speculative_draft_model_path="Jess437/Llama-3.2-1B-Instruct-gptq-b4g32"
    ...
    for i in tqdm(range(5), desc="Warm Up..."):
        ...
        generated = generate(model, input_ids, tokenizer)
