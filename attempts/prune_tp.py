# Source: https://github.com/VainF/Torch-Pruning/blob/master/examples/LLMs/prune_llm.py
...
def get_wikitext2(nsamples, seed, seqlen, tokenizer):
    ...
    testdata = load_dataset('wikitext', 'wikitext-2-raw-v1', split='validation')
...
def main():
    ...
    # importance = tp.importance.ActivationImportance(p=2, target_types=[torch.nn.Linear])
    pruner = tp.pruner.BasePruner(
        ...
        ignored_layers=[
            model.lm_head, 
            *[model.model.layers[x].self_attn for x in range(len(model.model.layers))]
        ],
        num_heads=num_heads,
