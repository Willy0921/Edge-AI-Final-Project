...
def generate(model, input_ids):
    sampling_params = SamplingParams(temperature=0, top_p=1, max_tokens=256+16)
    input_ids = input_ids.clone()
    input_ids = input_ids[0].tolist()
    output = model.generate(prompt_token_ids = input_ids, sampling_params = sampling_params)
    output = torch.tensor(output[0].outputs[0].token_ids).view(1,-1) # compatible with TA's code
...
def main():
    ...
    model_name = "Jess437/Llama-3.2-3B-Instruct-gptq-b4g32-p20"
    model = CallableLLM(model=model_name, 
                        dtype="half", 
    ...
    for i in tqdm(range(5), desc="Warm Up..."):
        ...
        generated = generate(model, input_ids)
